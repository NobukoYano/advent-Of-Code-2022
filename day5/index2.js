...
const calculate = (inputs) => {
    ...
    for (const move of formatted) {
        ...
        // diagonal move
        const horizontalMove = move[1][0] - move[0][0];
        const verticalMove = move[1][1] - move[0][1];

        // @TODO dynamically assign operator + or -
        if (horizontalMove > 0 && verticalMove > 0) {
            for (let i = 0; i <= Math.abs(horizontalMove); i++) {
                matrix[move[0][1] + i][move[0][0] + i] += 1;
            }
        }
...
const convertToArray = (inputs) => {
    ...
    for (const input of inputs) {
        ...
        const horizontalMove = Math.abs(move[0][0] - move[1][0]);
        const verticalMove = Math.abs(move[0][1] - move[1][1]);

        // Filter horizontal or vertical or diagonal move
        if (move[0][0] === move[1][0] || move[0][1] === move[1][1] || horizontalMove === verticalMove) {
            formatted.push(move);
